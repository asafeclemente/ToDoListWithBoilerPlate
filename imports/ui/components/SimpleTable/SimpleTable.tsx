...
export const SimpleTable = React.memo((props: ISimpleTable) => {
	...
	const renderType = (type: string, data: any, colName: SchemaKey) => {
		...
		if (type === 'image') {
			...
			return Array.isArray(data) ? (
				data.map((d, idx) => (
					<div key={'dgrs' + idx} dangerouslySetInnerHTML={{ __html: d }} />
				))
			) : (
			...
			const schemaData =
				...
				schema[colName].options.find(
					(object: any) => object.value === data || object === data
				);
	...
	const tableBody = (
		filter
			? newDate?.filter((d: { [key: string]: any }) => d[filterByField!] === filter)
			: newDate
				...
				{actions ? (
					<TableCell style={simpleTableStyle.tableCellActions}>
						{actions.map((act: IAct) => {
							// Verifica se o createdby é igual ao _id do usuário atual
							if (row.createdby === getUser()._id) {
								return (
									<IconButton
										key={act.icon + act.id}
										onClick={(evt) => {
							...
							return null; // Não renderiza a ação se createdBy for diferente do _id do usuário atual
						})}
					</TableCell>
				) : null}
